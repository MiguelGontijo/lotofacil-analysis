...
TEST_DATA_SORTEIOS = [
    # ... (Dados dos 20 concursos como definidos anteriormente - omitido por brevidade) ...
    ...
    {'concurso': 8,  'data_sorteio': '2023-01-08', 'b1': 6,  'b2': 7,  'b3': 8,  'b4': 9,  'b5': 10, 'b6': 11, 'b7': 12, 'b8': 13, 'b9': 14, 'b10': 15, 'b11': 1,  'b12': 3,  'b13': 5,  'b14': 7,  'b15': 9}, # 13 bolas
...
EXPECTED_CHUNK_10_STATS_CONC_10 = { # (Valores como antes)
...
EXPECTED_CHUNK_10_STATS_CONC_20 = { # (Valores como antes)
...
def populated_db_conn(test_db_conn: sqlite3.Connection) -> sqlite3.Connection:
    ...
    freq_col_defs = ', '.join([f'd{i}_freq INTEGER NOT NULL' for i in ALL_NUMBERS])
    rank_col_defs = ', '.join([f'd{i}_rank INTEGER NOT NULL' for i in ALL_NUMBERS])
    create_chunk_sql = f"""
    ...
    cursor.execute(create_chunk_sql); conn.commit()
