...
# Importa constantes DO PROJETO (config.py)
try:
    ...
    # Fallbacks caso config não esteja acessível
    print("WARN: conftest.py using Fallbacks for config constants.")
...
# Importa APENAS funções necessárias de outros módulos (se houver)
# Evita importar TUDO para não criar dependências complexas no setup de teste
...
NUM_TEST_RECORDS = len(TEST_DATA_SORTEIOS) # Exporta para outros testes
...
def populated_db_conn(test_db_conn: sqlite3.Connection) -> sqlite3.Connection:
    """ Popula BD em memória com sorteios e stats finais do chunk 10. """
    ...
    values_conc_10 = [EXPECTED_CHUNK_10_STATS_CONC_10.get(col, 0) for col in chunk_stat_cols]; cursor.execute(sql_insert_chunk, values_conc_10); conn.commit()
    print(f"Fixture: Inseriu 1 registro (concurso {EXPECTED_CHUNK_10_STATS_CONC_10['concurso_fim']}) em '{table_name_chunk_10}'.")
