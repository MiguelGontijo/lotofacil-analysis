...
# Importa DB manager apenas para mockar a conexão ou usar fixture
from src.database_manager import DATABASE_PATH
from unittest.mock import patch

# Importa dados esperados do conftest
from .conftest import EXPECTED_CHUNK_10_STATS_CONC_10, EXPECTED_CHUNK_10_STATS_CONC_20, ALL_NUMBERS
...
# --- Testes para get_chunk_final_stats (Mantidos como antes) ---
...
def test_get_chunk_final_stats_with_data(mock_connect, populated_db_conn: sqlite3.Connection):
    ...
    interval = 10
    result_df = get_chunk_final_stats(interval_size=interval, concurso_maximo=None) # Pede todos (10 e 20)

    assert result_df is not None; assert isinstance(result_df, pd.DataFrame)
    assert not result_df.empty; assert result_df.index.tolist() == [10, 20]; assert len(result_df) == 2

    # Verifica concurso 10
    row_10 = result_df.loc[10]
    assert row_10['d1_freq'] == EXPECTED_CHUNK_10_STATS_CONC_10['d1_freq']; assert row_10['d1_rank'] == EXPECTED_CHUNK_10_STATS_CONC_10['d1_rank']
    assert row_10['d25_freq'] == EXPECTED_CHUNK_10_STATS_CONC_10['d25_freq']; assert row_10['d25_rank'] == EXPECTED_CHUNK_10_STATS_CONC_10['d25_rank']

    # Verifica concurso 20
    row_20 = result_df.loc[20]
    assert row_20['d1_freq'] == EXPECTED_CHUNK_10_STATS_CONC_20['d1_freq']; assert row_20['d1_rank'] == EXPECTED_CHUNK_10_STATS_CONC_20['d1_rank']
    assert row_20['d25_freq'] == EXPECTED_CHUNK_10_STATS_CONC_20['d25_freq']; assert row_20['d25_rank'] == EXPECTED_CHUNK_10_STATS_CONC_20['d25_rank']
...
def test_get_chunk_final_stats_missing_table(mock_connect, test_db_conn: sqlite3.Connection):
    ...
    assert result_df is None # Espera None se a tabela não existe


# --- NOVO TESTE para calculate_historical_rank_stats ---
@patch('src.analysis.chunk_analysis.DATABASE_PATH', ':memory:')
@patch('src.analysis.chunk_analysis.sqlite3.connect')
def test_calculate_historical_rank_stats(mock_connect, populated_db_conn: sqlite3.Connection):
    ...
    hist_stats_df = calculate_historical_rank_stats(interval_size=interval, concurso_maximo=20)
    ...
    assert f'avg_rank_chunk{interval}' in hist_stats_df.columns
    assert f'std_rank_chunk{interval}' in hist_stats_df.columns

    # Verifica valores calculados manualmente para algumas dezenas
    # Dezena 1: Ranks [1, 9]. Avg=5.0. Std=sqrt(32)=5.657
    assert hist_stats_df.loc[1, f'avg_rank_chunk{interval}'] == pytest.approx(5.0)
    assert hist_stats_df.loc[1, f'std_rank_chunk{interval}'] == pytest.approx(np.sqrt(32))

    # Dezena 2: Ranks [3, 1]. Avg=2.0. Std=sqrt(2)=1.414
    assert hist_stats_df.loc[2, f'avg_rank_chunk{interval}'] == pytest.approx(2.0)
    assert hist_stats_df.loc[2, f'std_rank_chunk{interval}'] == pytest.approx(np.sqrt(2))

    # Dezena 25: Ranks [16, 9]. Avg=12.5. Std=sqrt(24.5)=4.950
    assert hist_stats_df.loc[25, f'avg_rank_chunk{interval}'] == pytest.approx(12.5)
    assert hist_stats_df.loc[25, f'std_rank_chunk{interval}'] == pytest.approx(np.sqrt(24.5))

    # Verifica se não há NaNs (pois temos 2 pontos para todos)
    assert not hist_stats_df.isnull().any().any()

def test_calculate_historical_rank_stats_insufficient_data():
    """ Testa o caso com menos de 2 chunks (std dev deve ser NaN). """
    # Mock get_chunk_final_stats para retornar apenas 1 linha
    mock_df = pd.DataFrame([EXPECTED_CHUNK_10_STATS_CONC_10]).set_index('concurso_fim')
    with patch('src.analysis.chunk_analysis.get_chunk_final_stats', return_value=mock_df):
