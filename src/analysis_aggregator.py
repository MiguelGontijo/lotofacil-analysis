...
# Usa AGGREGATOR_WINDOWS do config
from src.config import logger, ALL_NUMBERS, AGGREGATOR_WINDOWS, TREND_SHORT_WINDOW, TREND_LONG_WINDOW
...
# Importa a nova função de atraso intra-ciclo
from src.analysis.cycle_analysis import get_cycles_df, calculate_current_incomplete_cycle_stats, calculate_current_intra_cycle_delay
...
if 'AGGREGATOR_WINDOWS' not in globals(): AGGREGATOR_WINDOWS = [10, 25, 50, 100, 200] # Manter default menor se config falhar
...
def get_consolidated_analysis(concurso_maximo: int) -> Optional[Dict[str, Any]]:
    """ Executa análises e retorna V5 (mais janelas, atraso intra-ciclo). """
    logger.info(f"Agregando análises (v5) até o concurso {concurso_maximo}...")
    if concurso_maximo <= 0: logger.error("Concurso máximo inválido."); return None

    results: Dict[str, Any] = {}; errors_summary = {}

    # 1. Frequência Geral (Otimizada)
    ...
    # 2. Frequência Recente (Usa AGGREGATOR_WINDOWS do config)
    ...
    if delay_stats_df is not None:
        results['delay_mean'] = delay_stats_df['media_atraso']
        results['delay_std_dev'] = delay_stats_df['std_dev_atraso']
        results['max_delay'] = delay_stats_df['max_atraso']
    ...
    # *** NOVO: Atraso Intra-Ciclo Atual ***
    current_intra_cycle_delay = None
    if curr_cycle_start is not None: # Só calcula se existe ciclo atual
        current_intra_cycle_delay = calculate_current_intra_cycle_delay(curr_cycle_start, concurso_maximo)
    results['current_intra_cycle_delay'] = current_intra_cycle_delay
    if current_intra_cycle_delay is None and curr_cycle_start is not None: # Se o cálculo falhou mas havia ciclo
        errors_summary['intra_cycle_delay'] = "Falha"
    ...
    else: errors_summary['freq_trend'] = f"Faltam W{TREND_SHORT_WINDOW}/{TREND_LONG_WINDOW}"
