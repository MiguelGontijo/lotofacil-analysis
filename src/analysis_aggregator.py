...
# Importa APENAS o logger do config (e outras configs se precisar DEPOIS)
from src.config import logger # REMOVIDO ALL_NUMBERS DAQUI
...
# Janelas necessárias para o cálculo de tendência e score
AGGREGATOR_WINDOWS = [10, 25, 50, 100, 200] # Adicionado 100, 200
TREND_SHORT_WINDOW = 10
TREND_LONG_WINDOW = 50


def get_consolidated_analysis(concurso_maximo: int) -> Optional[Dict[str, Any]]:
    """ Executa análises e retorna resultados consolidados V3 (com mais janelas, trend, delay stats). """
    logger.info(f"Agregando análises (v3) até o concurso {concurso_maximo}...")
    ...
    if delay_stats_df is not None:
        ...
        errors_summary['delay_stats'] = "Falha"
        results['delay_mean'] = None; results['delay_std_dev'] = None; results['max_delay'] = None
    ...
    if freq_short is not None and freq_long is not None:
         # Usa ALL_NUMBERS definido localmente
    ...
    logger.info(f"Agregação de análises (v3) até {concurso_maximo} concluída.")
    return results