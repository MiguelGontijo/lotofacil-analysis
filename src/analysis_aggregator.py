...
# Importa a NOVA função get_cumulative_frequency e a calculate_frequency original
from src.analysis.frequency_analysis import (
    get_cumulative_frequency, # <<< Para overall_freq otimizado
    calculate_frequency as calculate_period_frequency, # <<< Para freq dentro de ciclo
...
AGGREGATOR_WINDOWS = [10, 25, 50, 100, 200]
...
def get_consolidated_analysis(concurso_maximo: int) -> Optional[Dict[str, Any]]:
    """ Executa análises e retorna resultados V4 (usa snapshots para freq geral). """
    logger.info(f"Agregando análises (v4) até {concurso_maximo}...")
    ...
    else: errors_summary['delay_stats'] = "Falha"; results['delay_mean']=None; results['delay_std_dev']=None; results['max_delay']=None
    ...
    if cycles_df_until_max is not None and not cycles_df_until_max.empty:
        ...
        if not completed_before_max.empty:
            last_cycle = completed_before_max.iloc[-1]; start_c, end_c = int(last_cycle['concurso_inicio']), int(last_cycle['concurso_fim']); logger.info(f"Calculando freq. do último ciclo: {last_cycle['numero_ciclo']}");
            # Usa calculate_period_frequency aqui pois é um range específico
    ...
    logger.info(f"Agregação de análises (v4) até {concurso_maximo} concluída.")
    return results