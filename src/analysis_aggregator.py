...
from src.analysis.frequency_analysis import (
    get_cumulative_frequency, # Usa snapshots para freq geral
    calculate_frequency as calculate_period_frequency, # Para ranges específicos
    calculate_windowed_frequency
)
from src.analysis.delay_analysis import calculate_current_delay, calculate_delay_stats
# Importa funções de ciclo, INCLUINDO a nova de atraso intra-ciclo
from src.analysis.cycle_analysis import (
    get_cycles_df,
    calculate_current_incomplete_cycle_stats,
    calculate_current_intra_cycle_delay # <<< IMPORT ADICIONADO
)
...
def get_consolidated_analysis(concurso_maximo: int) -> Optional[Dict[str, Any]]:
    """ Executa análises V5 (mais janelas, trend, delay stats, atraso intra-ciclo). """
    ...
    if cycles_df_until_max is not None and not cycles_df_until_max.empty:
        ...
        if not completed_before_max.empty:
            ...
            last_cycle_freq = calculate_period_frequency(concurso_minimo=start_c, concurso_maximo=end_c) # Usa func não otimizada (range curto)
    ...
    # Calcula Atraso Intra-Ciclo Atual (agora importado corretamente)
    current_intra_cycle_delay = None
    if curr_cycle_start is not None:
    ...
    if current_intra_cycle_delay is None and curr_cycle_start is not None: errors_summary['intra_cycle_delay'] = "Falha"
