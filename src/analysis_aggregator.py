...
# <<< Importa a nova análise de grupo >>>
from src.analysis.group_trend_analysis import calculate_group_freq_stats, DEFAULT_GROUP_WINDOWS
# Importa DB manager
...
def get_consolidated_analysis(concurso_maximo: int) -> Optional[Dict[str, Any]]:
    """ Executa análises V8 (inclui stats de grupo). """
    logger.info(f"Agregando análises (v8) até o concurso {concurso_maximo}...")
    ...
    windows_to_calc = set(AGGREGATOR_WINDOWS) | {TREND_SHORT_WINDOW, TREND_LONG_WINDOW} | set(DEFAULT_GROUP_WINDOWS) # Garante janelas p/ grupo
    for window in sorted(list(windows_to_calc)):
        key = f'recent_freq_{window}'
        # Reutiliza se já calculado para tendência/grupo
    ...
    last_cycle_freq = None; hist_intra_delay_df = None; closing_stats_df = None
    if cycles_df_until_max is not None and not cycles_df_until_max.empty:
        ...
        # hist_intra_delay_df = calculate_historical_intra_cycle_delay_stats(cycles_df_until_max) # Pode ser lento
    ...
    if freq_short is not None and freq_long is not None:
         freq_trend = pd.Series(np.where(freq_long == 0, np.where(freq_short > 0, 999, 1), freq_short / freq_long), index=ALL_NUMBERS).fillna(1).reindex(ALL_NUMBERS, fill_value=1.0)
    ...
    # *** 7. NOVO: Tendências de Grupo ***
    results['group_freq_stats'] = calculate_group_freq_stats(concurso_maximo)
    if results['group_freq_stats'] is None: errors_summary['group_stats'] = "Falha"


    if errors_summary: logger.error(f"Erros na agregação: {errors_summary}")
    logger.info(f"Agregação de análises (v8) até {concurso_maximo} concluída.")
    return results