...
from src.config import (
    logger, ALL_NUMBERS, AGGREGATOR_WINDOWS, # Usa AGGREGATOR_WINDOWS do config
...
def get_consolidated_analysis(concurso_maximo: int) -> Optional[Dict[str, Any]]:
    """ Executa análises V6 (mais janelas, hist intra-delay, repetição N-1). """
    logger.info(f"Agregando análises (v6) até o concurso {concurso_maximo}...")
    ...
    if cycles_df_until_max is not None and not cycles_df_until_max.empty:
        ...
        if not completed_before_max.empty:
            last_cycle = completed_before_max.iloc[-1]; start_c, end_c = int(last_cycle['concurso_inicio']), int(last_cycle['concurso_fim']); logger.info(f"Calculando freq. do último ciclo: {last_cycle['numero_ciclo']}"); last_cycle_freq = calculate_period_frequency(concurso_minimo=start_c, concurso_maximo=end_c)
    ...
    # *** NOVO: Stats Históricos de Atraso Intra-Ciclo ***
    hist_intra_delay_df = calculate_historical_intra_cycle_delay_stats(cycles_df_until_max) # Passa os ciclos completos
    if hist_intra_delay_df is not None:
        ...
        results['max_hist_intra_delay'] = hist_intra_delay_df['max_hist_intra_delay']
         ...
         errors_summary['hist_intra_delay'] = "Falha"
         results['avg_hist_intra_delay'] = None
         results['max_hist_intra_delay'] = None
    ...
    # *** NOVO: Números do Último Sorteio (N-1) ***
    if concurso_maximo > 0:
         ...
         results['numbers_in_last_draw'] = set() # Vazio se concurso_maximo for 0 ou menos
