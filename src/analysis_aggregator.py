...
# Importa funções de análise (incluindo a otimizada get_cumulative_frequency)
...
from src.analysis.cycle_analysis import (
    get_cycles_df, calculate_current_incomplete_cycle_stats,
    calculate_current_intra_cycle_delay, calculate_historical_intra_cycle_delay_stats
)
...
def get_consolidated_analysis(concurso_maximo: int) -> Optional[Dict[str, Any]]:
    """ Executa análises V6 (usa snapshots para freq geral). """
    logger.info(f"Agregando análises (v6) até {concurso_maximo}...")
    ...
    results['overall_freq'] = get_cumulative_frequency(concurso_maximo=concurso_maximo) # <<< USA SNAPSHOTS
    if results['overall_freq'] is None: errors_summary['overall_freq'] = "Falha"

    # 2. Frequência Recente (Calculada na hora)
    ...
    if cycles_df_until_max is not None and not cycles_df_until_max.empty:
        ...
        if not completed_before_max.empty:
            last_cycle = completed_before_max.iloc[-1]; start_c, end_c = int(last_cycle['concurso_inicio']), int(last_cycle['concurso_fim']); logger.info(f"Calculando freq. últ. ciclo: {last_cycle['numero_ciclo']}");
            last_cycle_freq = calculate_period_frequency(concurso_minimo=start_c, concurso_maximo=end_c)
        # Calcula stats históricos intra-ciclo usando o DF de ciclos completos
        hist_intra_delay_df = calculate_historical_intra_cycle_delay_stats(cycles_df_until_max)
        if hist_intra_delay_df is not None: results['avg_hist_intra_delay']=hist_intra_delay_df['avg_hist_intra_delay']; results['max_hist_intra_delay']=hist_intra_delay_df['max_hist_intra_delay'] # Adiciona std dev se calculado
        else: errors_summary['hist_intra_delay'] = "Falha"; results.update({'avg_hist_intra_delay':None, 'max_hist_intra_delay':None})
    else: # Se não há ciclos, não há stats históricos intra-ciclo
        results['avg_hist_intra_delay'] = None; results['max_hist_intra_delay'] = None
    ...
    logger.info(f"Agregação de análises (v6) até {concurso_maximo} concluída.")
    return results