...
from src.config import (
    ...
    FREQ_CHUNK_DETAIL_PREFIX, # <<< Novo prefixo
...
def read_data_from_db(db_path: Path = DATABASE_PATH, table_name: str = TABLE_NAME, columns: Optional[List[str]] = None, concurso_minimo: Optional[int] = None, concurso_maximo: Optional[int] = None) -> Optional[pd.DataFrame]:
    ...
    except Exception as e: logger.error(f"Erro ler '{table_name}': {e}"); return None
...
def create_cycles_table(db_path: Path = DATABASE_PATH):
    try:
        with sqlite3.connect(db_path) as conn: sql_create = f"""CREATE TABLE IF NOT EXISTS {CYCLES_TABLE_NAME} (...);"""; conn.execute(sql_create); logger.info(f"Tabela '{CYCLES_TABLE_NAME}' ok.")
...
# --- NOVAS FUNÇÕES PARA TABELAS DE CHUNK DETALHADAS ---
def get_chunk_table_name(interval_size: int) -> str:
    """ Gera o nome da tabela de chunk detalhada para um intervalo. """
...
def create_chunk_freq_table(interval_size: int, db_path: Path = DATABASE_PATH):
    """ Cria a tabela de frequência detalhada por chunk para um intervalo específico. """
...
def get_last_contest_in_chunk_table(interval_size: int, db_path: Path = DATABASE_PATH) -> Optional[int]:
    """ Busca o último concurso registrado na tabela de chunk especificada. """
    table_name = get_chunk_table_name(interval_size)
    create_chunk_freq_table(interval_size, db_path) # Garante que exista
    try:
        with sqlite3.connect(db_path) as conn:
            result = conn.execute(f"SELECT MAX(concurso) FROM {table_name}").fetchone()
...
def save_chunk_freq_row(concurso: int, freq_counts_series: pd.Series, interval_size: int, db_path: Path = DATABASE_PATH):
    """ Salva (INSERT OR REPLACE) uma linha na tabela de chunk detalhada. """
    table_name = get_chunk_table_name(interval_size)
    create_chunk_freq_table(interval_size, db_path) # Garante que exista
    if freq_counts_series is None or len(freq_counts_series.index.intersection(ALL_NUMBERS)) != 25:
        logger.error(f"Série freq inválida p/ salvar chunk {interval_size}, concurso {concurso}.")
        return
    logger.debug(f"Salvando freq chunk {interval_size} para concurso {concurso}...")
    try:
        with sqlite3.connect(db_path) as conn:
            col_names = ['concurso'] + [f'd{i}' for i in ALL_NUMBERS]
            placeholders = ', '.join(['?'] * (len(ALL_NUMBERS) + 1))
            values_ordered = [freq_counts_series.get(i, 0) for i in ALL_NUMBERS]
            sql = f"INSERT OR REPLACE INTO {table_name} ({', '.join(col_names)}) VALUES ({placeholders});"
            values = [concurso] + values_ordered
            conn.execute(sql, values); conn.commit()
    except sqlite3.Error as e:
        logger.error(f"Erro SQLite ao salvar chunk {interval_size} para concurso {concurso}: {e}")
    except Exception as e:
         logger.error(f"Erro inesperado ao salvar chunk {interval_size} para concurso {concurso}: {e}")

def get_chunk_freq_row(concurso: int, interval_size: int, db_path: Path = DATABASE_PATH) -> Optional[pd.Series]:
    """ Lê uma linha específica (frequências para um concurso) da tabela de chunk. """
    table_name = get_chunk_table_name(interval_size)
    logger.debug(f"Lendo freq chunk {interval_size} para concurso {concurso}...")
    try:
        with sqlite3.connect(db_path) as conn:
            col_names = [f'd{i}' for i in ALL_NUMBERS]
            sql = f"SELECT {', '.join(col_names)} FROM {table_name} WHERE concurso = ?"
            result = conn.execute(sql, (concurso,)).fetchone()
            if result:
                freq_data = {num: int(count or 0) for num, count in zip(ALL_NUMBERS, result)}
                return pd.Series(freq_data).astype(int)
            else:
                return None # Linha não encontrada
    except sqlite3.Error as e:
        logger.error(f"Erro ao ler chunk {interval_size} para concurso {concurso}: {e}")
        return None