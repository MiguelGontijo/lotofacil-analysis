...
# Importa a NOVA estratégia de score
from src.strategies.scoring_strategies import select_top_scored
...
class AnalysisOrchestrator:
    ...
    def _setup_parser(self) -> argparse.ArgumentParser:
        ...
        # Adiciona 'top_score' às escolhas
        backtest_group.add_argument('--strategy', type=str,
                                    choices=['most_freq', 'least_freq', 'most_freq_recent', 'most_delayed', 'top_score'],
        ...
        parser.add_argument('--predict-last', action='store_true', help="Roda análises N-1 e compara com N.")
        return parser
    ...
    def _run_backtest_mode(self):
        ...
        # Atualiza o mapeamento para incluir a nova estratégia
        strategy_map: Dict[str, StrategyFuncType] = {
            ...
            'top_score':        lambda max_c: select_top_scored(max_c) # <<< Nova estratégia
        }
        ...
        # AVISO SOBRE PERFORMANCE para estratégia de score
        if self.args.strategy == 'top_score':
             num_concursos = end_contest - start_contest + 1
             if num_concursos > 200: # Limite arbitrário para aviso
                  self.logger.warning(f"A estratégia 'top_score' recalcula múltiplas análises por concurso.")
                  self.logger.warning(f"Testar {num_concursos} concursos pode ser MUITO LENTO. Considere um range menor (--start, --end).")
        ...
        if backtest_summary:
            ...
            if self.should_plot:
                ...
                global PLOTTING_ENABLED # Acessa flag global
    ...
    def _run_predict_last_mode(self):
        ...
        # Calcula os scores usando o scorer
        scores = calculate_scores(analysis_results) # Usa config padrão
        ...
        if not scores.empty:
             ...
             print(f"\n--- Dezenas Selecionadas (Score até {n_minus_1}) ---"); print(sorted(list(selected_numbers)))
        else: self.logger.error("Não foi possível calcular ou rankear scores.")
        ...
        if selected_numbers is not None:
             ...
             print(f"\n--- Comparação ---"); print(f"Acertos da seleção (Score) vs. Concurso {n}: {hits} pontos")
