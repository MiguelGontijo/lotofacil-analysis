...
def main():
    ...
    try:
        ...
        # Adiciona o próprio contexto para que os steps possam acessá-lo se precisarem de flexibilidade
        shared_context_for_orchestrator["shared_context"] = shared_context_for_orchestrator

        # Os nomes na lista "args" devem corresponder aos parâmetros das funções de step.
        # Todas as funções de step devem aceitar os parâmetros listados e `**kwargs` ou `shared_context` para flexibilidade.
        ...
        # ... (lógica de seleção de pipeline como antes) ...
        if args.run_steps:
        ...
        elif force_reload_data:
            pipeline_to_run_config = main_analysis_pipeline_config
        else:
            logger.info("Nenhuma ação. Use --run-steps ou --force-reload. Veja --help.")
            parser.print_help()
            return
        ...
        if pipeline_to_run_config:
            ...
            orchestrator.run()
             ...
             logger.info("Nenhuma etapa válida selecionada para execução.")
