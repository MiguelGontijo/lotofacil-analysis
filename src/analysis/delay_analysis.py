...
import pandas as pd
from typing import Optional, Dict, List # Adicionado List
...
def calculate_current_delay(concurso_maximo: Optional[int] = None) -> Optional[pd.Series]:
    ...
    if pd.isna(effective_max_concurso): # Checagem adicional
         logger.error("Não foi possível determinar o concurso máximo efetivo.")
         return None
    effective_max_concurso = int(effective_max_concurso)
...
def calculate_max_delay(concurso_maximo: Optional[int] = None) -> Optional[pd.Series]:
    """
    Calcula o atraso máximo histórico (maior sequência sem sorteio) para
    cada dezena (1-25), até um concurso máximo especificado.
    """
    logger.info(f"Calculando atraso máximo histórico até o concurso {concurso_maximo or 'último'}...")
    df = read_data_from_db(columns=BASE_COLS, concurso_maximo=concurso_maximo)
    if df is None or df.empty:
    ...
    for n in ALL_NUMBERS:
         if last_seen_concurso[n] >= first_concurso: # Só calcula se já foi visto
             ...
             logger.warning(f"Dezena {n} nunca vista até {effective_max_concurso}. Atraso máx considerado total.")
             # O atraso máximo é o próprio período, pois nunca apareceu
