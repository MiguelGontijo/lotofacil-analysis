...
import numpy as np # Import crucial
...
def aggregate_block_data_to_wide_format(db_manager: DatabaseManager):
    # ... (código completo desta função como fornecido na penúltima resposta) ...
    # Esta função lida com a agregação de dados de CHUNKS/BLOCOS
...
def aggregate_cycle_data_to_wide_format(db_manager: DatabaseManager):
    # ... (código completo desta função como fornecido na penúltima resposta) ...
    # Esta função lida com a agregação de dados de CICLOS
    ...
    if df_ciclos_detalhe is None or df_ciclos_detalhe.empty: logger.warning("Tabela 'ciclos_detalhe' não encontrada ou vazia."); return
    df_ciclos_base_info = df_ciclos_detalhe[['ciclo_num', 'concurso_inicio', 'concurso_fim', 'duracao_concursos']].copy()
    df_ciclos_base_info.rename(columns={'concurso_inicio': 'concurso_inicio_ciclo', 'concurso_fim': 'concurso_fim_ciclo', 'duracao_concursos': 'duracao_ciclo'}, inplace=True)
    ...
    if all_wide_dfs_for_cycles:
        ...
        except Exception as e: logger.error(f"Erro ao salvar tabela consolidada de CICLOS '{consolidated_cycle_table_name}': {e}", exc_info=True)
    else: logger.warning(f"Nenhum DataFrame largo gerado para CICLOS. Tabela '{consolidated_cycle_table_name}' não foi criada.")
