...
def aggregate_cycle_data_to_wide_format(db_manager: DatabaseManager):
    logger.info("Iniciando agregação de dados de CICLO para formato largo (incluindo rank e métricas de grupo).") # Log Atualizado
    ...
    cycle_group_metrics_table_name = "ciclo_group_metrics" # <<< NOVA TABELA A SER LIDA
    ...
    if df_cycle_group_metrics is not None:
        if not df_consolidated_cycles_wide.empty:
            # Merge com as métricas de grupo do ciclo.
            # Chave de merge é 'ciclo_num'.
            # As colunas avg_pares_no_ciclo, etc., serão adicionadas a todas as linhas de tipo_analise_ciclo para o mesmo ciclo_num.
            if 'ciclo_num' in df_consolidated_cycles_wide.columns and 'ciclo_num' in df_cycle_group_metrics.columns:
                ...
                logger.warning("Coluna 'ciclo_num' não encontrada em um dos DataFrames para merge de métricas de grupo de ciclo.")
        else: 
            logger.info("Construindo tabela consolidada de ciclos apenas com métricas de grupo (pois não havia métricas por dezena).")
            # Merge df_closed_ciclos_base_info (que tem ciclo_num, inicio, fim, duracao) com df_cycle_group_metrics
            if 'ciclo_num' in df_closed_ciclos_base_info.columns and 'ciclo_num' in df_cycle_group_metrics.columns:
                ...
                # Esta tabela não terá 'tipo_analise_ciclo' nem 'dezena_X' colunas.
                # Pode ser preferível não criar a tabela consolidada se não houver métricas por dezena.
                # Ou, adicionar uma linha com tipo_analise = "metricas_grupo_ciclo"
                # Para manter a estrutura, vamos pular se df_all_wide_dfs_for_cycles estava vazio.
                if not all_wide_dfs_for_cycles: # Se não havia métricas por dezena
                    logger.warning("Nenhuma métrica por dezena foi agregada para ciclos. Tabela consolidada de ciclos não será criada apenas com métricas de grupo neste fluxo.")
                    df_consolidated_cycles_wide = pd.DataFrame() # Reseta para não salvar
