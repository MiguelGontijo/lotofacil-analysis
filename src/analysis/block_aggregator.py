...
import numpy as np # <<< IMPORT ADICIONADO AQUI
...
def aggregate_block_data_to_wide_format(db_manager: DatabaseManager):
    ...
    metric_configs = [
        ...
        { 
            "source_table_prefix": "evol_metric_atraso_maximo_bloco",
            "value_column": "atraso_maximo_no_bloco",
            "analysis_type_name": "atraso_maximo_bloco",
            "dtype": "Int64"
        },
        { 
            "source_table_prefix": "evol_metric_atraso_final_bloco",
            "value_column": "atraso_final_no_bloco",
            "analysis_type_name": "atraso_final_bloco",
            "dtype": "Int64"
        },
    ]
        ...
        for size in chunk_sizes:
            ...
            for metric_config in metric_configs:
                ...
                if not all(col in df_long.columns for col in required_for_pivot):
                    logger.error(f"Tabela '{long_format_table_name}' não tem colunas para pivotar ({required_for_pivot}). Colunas: {df_long.columns.tolist()}. Pulando '{analysis_type_name_for_wide_table}'.")
                    continue
                ...
                try:
                    ...
                    df_wide_metric = df_long.pivot_table(
                        ...
                        fill_value = pd.NA if expected_dtype_str == "Int64" else np.nan # np.nan já está definido
                    )
    ...
    logger.info("Agregação de dados de bloco para formato largo (incluindo novas métricas de atraso) concluída.")