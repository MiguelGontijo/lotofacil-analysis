...
# Importa do config (COM CYCLES_TABLE_NAME, SEM imports inválidos)
...
def _find_new_cycles_in_data(df_new_data: pd.DataFrame,
    ...
    for index, row in df_new_data.iterrows():
        ...
        if current_cycle_numbers == ALL_NUMBERS_SET: # Usa ALL_NUMBERS_SET do config
...
def update_cycles_table(force_rebuild: bool = False):
    ...
    # Usa TABLE_NAME (sorteios) e BASE_COLS
...
def get_cycles_df(concurso_maximo: Optional[int] = None) -> Optional[pd.DataFrame]:
    ...
    cycles_table_name = CYCLES_TABLE_NAME # Usa constante
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            if not df.empty:
                 # Adiciona conversão robusta para Inteiros Nulos do Pandas
...
def calculate_frequency_per_cycle(cycles_df: pd.DataFrame) -> Dict[int, Optional[pd.Series]]:
    ...
    # Importa localmente para clareza
    from src.analysis.frequency_analysis import calculate_frequency
    for index, row in cycles_df.iterrows():
        try:
            cycle_num = int(row['numero_ciclo']); start_c = int(row['concurso_inicio']); end_c = int(row['concurso_fim'])
            cycle_frequencies[cycle_num] = calculate_frequency(concurso_minimo=start_c, concurso_maximo=end_c)
        except (ValueError, TypeError, KeyError) as e: logger.error(f"Erro processar linha ciclo {index}: {row}. E: {e}"); cycle_frequencies[int(row.get('numero_ciclo',-1))] = None
...
def calculate_current_incomplete_cycle_stats(concurso_maximo: int) -> Tuple[Optional[int], Optional[Set[int]], Optional[pd.Series]]:
    ...
    if relevant_cycles is None : logger.error("Falha ao ler ciclos da tabela."); start_of_current_cycle = None
        ...
        # Se tabela de ciclos está vazia, busca o primeiro concurso em sorteios
        df_min = read_data_from_db(columns=['concurso'], concurso_maximo=concurso_maximo, table_name=TABLE_NAME); # Usa TABLE_NAME
        min_c = df_min['concurso'].min() if (df_min is not None and not df_min.empty) else None
        start_of_current_cycle = int(min_c) if pd.notna(min_c) else None
        logger.info(f"Nenhum ciclo completo na tabela até {concurso_maximo}. Ciclo atual inicia em {start_of_current_cycle}.")
    else:
        last_complete_cycle_end_val = relevant_cycles['concurso_fim'].max()
        start_of_current_cycle = int(last_complete_cycle_end_val + 1) if pd.notna(last_complete_cycle_end_val) else None
...
def calculate_current_intra_cycle_delay(current_cycle_start: Optional[int], concurso_maximo: int) -> Optional[pd.Series]:
    """ Calcula o atraso de cada dezena DENTRO do ciclo incompleto atual. """
    ...
    cycle_len_so_far = concurso_maximo - current_cycle_start + 1 # Calcula antes
    ...
    for number in ALL_NUMBERS:
        last_seen = last_seen_in_cycle.get(number)
        delays[number] = (concurso_maximo - last_seen) if last_seen is not None else cycle_len_so_far
...
def run_cycle_frequency_analysis(cycles_df: pd.DataFrame, num_cycles_each_end: int = 3):
    ...
    # *** Bloco de Impressão com Indentação CORRIGIDA ***
    for cycle_num, cycle_name in display_items:
        ...
        if freq_series is not None and not freq_series.empty:
            ...
            try:
                # Prints em linhas separadas com indentação correta
                print("Top 5 +:")
