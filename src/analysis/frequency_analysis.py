...
def calculate_frequency(concurso_minimo: Optional[int] = None,
    ...
    if df.empty: logger.warning(f"Nenhum dado {period_str}."); return pd.Series(0, index=ALL_NUMBERS)
...
def get_cumulative_frequency(concurso_maximo: int) -> Optional[pd.Series]:
    """ Obtém a frequência geral acumulada até concurso_maximo, usando snapshots. """
    ...
    base_freq = pd.Series(0, index=ALL_NUMBERS)

    if snapshot_info:
        snap_concurso, snap_freq = snapshot_info
        if snap_freq is None or len(snap_freq) != 25 or snap_freq.isnull().any():
             logger.warning(f"Snapshot inválido para {snap_concurso}. Recalculando do início.")
             start_contest_delta = 1; base_freq = pd.Series(0, index=ALL_NUMBERS)
        else:
             logger.debug(f"Usando snapshot do concurso {snap_concurso}.")
             base_freq = snap_freq.copy().astype(int)
             start_contest_delta = snap_concurso + 1
    else: logger.info("Nenhum snapshot encontrado, calculando frequência total do início...")
    ...
    if start_contest_delta > concurso_maximo:
        ...
        if delta_freq is None: logger.error(f"Falha calcular delta freq ({start_contest_delta}-{concurso_maximo})."); return None
